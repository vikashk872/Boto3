...
for each_rds in response:
    ...
    for each in property['DBSnapshotAttributesResult']['DBSnapshotAttributes']:
         print(" \t Snaphsot Available to {if value is all then its public} : ",end='')
         print( each['AttributeValues'] , "  " ,end='')
         if each['AttributeValues'] == ['all'] :
             print("Delete")
             rds.delete_db_snapshot(DBSnapshotIdentifier=each_rds['DBSnapshotIdentifier'])
             print("Deleted Public Snapshot: " +  each_rds['DBSnapshotIdentifier'], end ='')
         else:
             print("Don't Delete")